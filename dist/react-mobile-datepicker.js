...
function nextYear(now) {
    ...
    var year = now.getFullYear() + index;

    // 年份通常不需要循环，但这里保持原有逻辑
    var date = new Date(year, now.getMonth(), now.getDate(), now.getHours(), now.getMinutes(), now.getSeconds());
    return date;
}
...
function nextDate(now) {
    ...
    var date = new Date(year, month, day, now.getHours(), now.getMinutes(), now.getSeconds());
    return date;
}
